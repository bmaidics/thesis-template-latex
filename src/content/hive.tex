...
\begin{itemize}
	...
	\item Map-side join: if we join two tables and one of them is small, map-side join can be done. This way, the small table will be copied to the memory of all the Map nodes.
	\item \etc
\end{itemize}
...
\begin{itemize}
	\item When we have many table partitions, HS2 needs to load all the partition metadata from HMS. This can cause a real memory issue, and  HiveServer can run out of heap memory and crash.
